...
interface Props {
  data: { gender: string; count: number; employed: number }[]
}

export default function GenderChart({ data }: Props) {
  return (
    <div className="bg-white rounded-xl border border-slate-200 shadow-sm p-5">
      <h3 className="text-sm font-semibold text-slate-800 mb-4">성별 신청 / 취업완료</h3>
      {data.length === 0
        ...
        : <ResponsiveContainer width="100%" height={200}>
            <BarChart data={data} barGap={2} barSize={28}>
              ...
              <XAxis dataKey="gender" tick={{ fontSize: 12, fill: '#64748b' }} axisLine={false} tickLine={false} />
